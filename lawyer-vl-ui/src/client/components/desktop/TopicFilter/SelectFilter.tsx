...
export const SelectFilter: React.FC<Props> = (props: Props) => {
	const { filterValues, label, onSelect, chosenValue } = props
	const [selectedValue, setSelectedValue] = useState<any>(
		chosenValue ?? filterValues[0]
	)
											...
											<Listbox.Option key={value} value={value}>
												{({ selected, active }) => (
													...
													>
														...
														{selected && (
															<span
																className={`absolute inset-y-0 left-0 flex items-center pl-1.5 hover:bg-purple-700`}
															>
																<svg
																	className="h-5 w-5"
